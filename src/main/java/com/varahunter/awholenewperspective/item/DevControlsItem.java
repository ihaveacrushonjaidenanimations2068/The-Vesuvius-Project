...
import com.varahunter.awholenewperspective.gui.DevContGui;
import com.varahunter.awholenewperspective.AWholeNewPerspectiveModElements;

@AWholeNewPerspectiveModElements.ModElement.Tag
public class DevControlsItem extends AWholeNewPerspectiveModElements.ModElement {
	@ObjectHolder("a_whole_new_perspective:dev_controls")
	public static final Item block = null;
	public DevControlsItem(AWholeNewPerspectiveModElements instance) {
		super(instance, 79);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof DevContGui.GuiWindow) {
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Dev Controls");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new DevContGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
