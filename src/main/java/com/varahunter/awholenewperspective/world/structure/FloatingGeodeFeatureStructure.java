...
public class FloatingGeodeFeatureStructure extends AWholeNewPerspectiveModElements.ModElement {
	public FloatingGeodeFeatureStructure(AWholeNewPerspectiveModElements instance) {
		super(instance, 33);
	}
	...
	public void init(FMLCommonSetupEvent event) {
				...
				if ((random.nextInt(1000000) + 1) <= 10000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("a_whole_new_perspective", "subvolite_floating_geode"));
						if (template == null)
							return false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("a_whole_new_perspective:erratic_petrified_field")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
